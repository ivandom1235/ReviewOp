...
class SelectiveRouterV2:
    ...
    def _accept_threshold(self, ex: RuntimeExample) -> float:
    ...
    def route_one(self, ex: RuntimeExample, cand: CandidateScore, is_top_candidate: bool = False) -> CandidateScore:
        cfg = self.config
        accept_threshold = self._accept_threshold(ex)
    ...
    def route_candidates(self, ex: RuntimeExample, candidates: list[CandidateScore]) -> list[CandidateScore]:
        ...
        for i, cand in enumerate(candidates):
            ...
            threshold = self.config.class_thresholds.get(cand.aspect, self._accept_threshold(ex))
            if (
                cand.final_score >= threshold
                and cand.evidence_support >= self._known_evidence_floor(ex)
