...
export async function getBatchAspectGraph(filters = {}) { const p = new URLSearchParams(); if (filters.domain) p.set("domain", filters.domain); if (filters.product_id) p.set("product_id", filters.product_id); if (filters.from) p.set("from", filters.from); if (filters.to) p.set("to", filters.to); p.set("min_edge_weight", String(filters.min_edge_weight || 1)); if (filters.graph_mode) p.set("graph_mode", filters.graph_mode); if (filters.contradiction_type) p.set("contradiction_type", filters.contradiction_type); if (filters.quarantine_status) p.set("quarantine_status", filters.quarantine_status); if (filters.graph_support_score) p.set("graph_support_score", String(filters.graph_support_score)); return request(`/graph/aspects${p.toString() ? `?${p.toString()}` : ""}`); }
