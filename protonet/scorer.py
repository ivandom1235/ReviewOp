...
class ScoringContext:
    ...
    known_classifier: object | None = None
...
def apply_generic_label_penalty(
    aspect: str,
    final_score: float,
    lexical: float,
    description_support: float,
    proto_score: float,
    config: ECProtoNetV2Config,
) -> float:
    if aspect in {"quality", "value", "performance", "usability"}:
        if lexical < 0.25 and description_support < 0.25 and proto_score < 0.25:
            penalty = getattr(config, "generic_aspects_penalty_multiplier", 0.80)
...
def _score_candidate(
    ...
    candidate_source: str = "prototype",
) -> CandidateScore:
    cfg = context.config
    # Inference must remain evidence-blind: score from raw review text only.
    evidence_text = ex.text
    lexical = alias_evidence_support(evidence_text, aspect, context.label_aliases)
    ...
    if candidate_source == "classifier":
        final = (
            0.55 * float(classifier_prob)
            + 0.20 * float(description_support)
            + 0.15 * float(candidate_evidence)
            + 0.10 * float(prior)
        )
    else:
        final = (
            ...
            + (cfg.w_classifier * float(classifier_prob) if getattr(cfg, "use_known_classifier", False) else 0.0)
        ) / max(
            1e-6,
            cfg.w_proto
            + cfg.w_evidence
            ...
            + cfg.w_scope
            + cfg.w_prior
            + (cfg.w_classifier if getattr(cfg, "use_known_classifier", False) else 0.0),
        )

    final = apply_generic_label_penalty(
        aspect=aspect,
        final_score=final,
        lexical=lexical,
...
def score_examples(examples: list[RuntimeExample], context: ScoringContext, top_k: int | None = None) -> dict[str, list[CandidateScore]]:
    if not all(isinstance(ex, RuntimeExample) for ex in examples):
        raise TypeError("score_examples expects RuntimeExample only")
    ...
    classifier_label_to_idx = None
    if getattr(context.config, "use_known_classifier", False) and context.known_classifier is not None:
        classifier_probs = context.known_classifier.predict_proba(texts)
        classifier_label_to_idx = {l: i for i, l in enumerate(getattr(context.known_classifier, "labels", []))}
    ...
    for row_i, ex in enumerate(examples):
        ...
        for j in range(usable):
            ...
            cands.append(
                _score_candidate(
                    ...
                    next_proto,
                    context,
                    classifier_prob=float(classifier_probs[row_i, classifier_label_to_idx[aspect]])
                    if classifier_probs is not None and classifier_label_to_idx is not None and aspect in classifier_label_to_idx
                    else 0.0,
        ...
        if classifier_probs is not None and classifier_label_to_idx is not None:
            row_probs = classifier_probs[row_i]
            top_classifier_idx = np.argsort(-row_probs)[: max(top_k, 1)]
            for clf_idx in top_classifier_idx:
                aspect = context.known_classifier.labels[int(clf_idx)]
                ...
                cands.append(
                    _score_candidate(
                        ...
                        0.0,
                        context,
                        classifier_prob=float(row_probs[int(clf_idx)]),
...
def alias_evidence_support(text: str, aspect: str, aliases: dict[str, list[str]] | None) -> float:
    terms = [aspect.replace("_", " ")]
    for t in (aliases or {}).get(aspect, []):
        terms.append(str(t).replace("_", " "))
    vals = [evidence_support(text, t, matched_terms=None) for t in terms if t]
