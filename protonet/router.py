...
class SelectiveRouterV2:
    ...
    def route_one(self, ex: ReviewExample, cand: CandidateScore, is_top_candidate: bool = False) -> CandidateScore:
        cfg = self.config
    ...
    def route_candidates(self, ex: ReviewExample, candidates: list[CandidateScore]) -> list[CandidateScore]:
        if not candidates:
            return [
                CandidateScore(
                    aspect="unknown",
        ...
        if self._should_emit_open_world(ex, top):
            open_cand = self._make_open_world_candidate(
                ex,
                top,
                "open_world_known_space_rejected_with_evidence"
            )
            routed_known = [
                top.with_decision("needs_review", "known_top1_rejected_before_open_world")
            ]
            for cand in candidates[1:]:
                # Multi-label safety: even when open-world is emitted, we block other known labels
                routed_known.append(
                    cand.with_decision(
                        "needs_review",
                        "non_rank1_candidate_blocked_by_winner_aware_router"
                    )
                )
            return [open_cand] + routed_known
        ...
        for i, cand in enumerate(candidates):
            ...
            # Multi-label acceptance (Phase 5)
            # Allow non-rank-1 only if very strong and passes threshold
            threshold = self.config.class_thresholds.get(cand.aspect, self.config.accept_threshold)
            if cand.final_score >= threshold and cand.evidence_support >= max(0.50, self.config.known_label_evidence_floor):
                routed.append(cand.with_decision("accept_known", "multi_label_high_confidence_accept"))
